...
/*****************  发送字符串 **********************/
void Usart_SendString_with_size( USART_TypeDef * pUSARTx, uint8_t *str, uint16_t Size)
{
  uint16_t i = 0;
	for(i=0; i<Size; i++)
	{
...
void set_computer_value(uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
  ...
  Usart_SendString_with_size(DEBUG_USART, (uint8_t *)&set_packet, sizeof(set_packet));    // 发送数据头
  Usart_SendString_with_size(DEBUG_USART, (uint8_t *)data, num);                          // 发送参数
  Usart_SendString_with_size(DEBUG_USART, (uint8_t *)&sum, sizeof(sum));                  // 发送校验和
}
