...
//	RCC_AHB1PeriphClockCmd (GENERAL_OCPWM_GPIO_CLK, ENABLE); 
	RCC_AHB1PeriphClockCmd (GENERAL_ICPWM_GPIO_CLK, ENABLE); 	

	/* 定时器复用引脚 */
//	GPIO_PinAFConfig(GENERAL_OCPWM_GPIO_PORT,GENERAL_OCPWM_PINSOURCE,GENERAL_OCPWM_AF); 
	GPIO_PinAFConfig(GENERAL_ICPWM_GPIO_PORT,GENERAL_IC1PWM_PINSOURCE,GENERAL_ICPWM_AF);
  GPIO_PinAFConfig(GENERAL_ICPWM_GPIO_PORT,GENERAL_IC2PWM_PINSOURCE,GENERAL_ICPWM_AF); 	
...
//	GPIO_Init(GENERAL_OCPWM_GPIO_PORT, &GPIO_InitStructure);
	...
	/* 高级控制定时器PWM输入捕获引脚 */
	GPIO_InitStructure.GPIO_Pin = GENERAL_IC1PWM_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;	
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_DOWN;
	GPIO_Init(GENERAL_ICPWM_GPIO_PORT, &GPIO_InitStructure);
	
	
	GPIO_InitStructure.GPIO_Pin = GENERAL_IC2PWM_PIN;
	GPIO_Init(GENERAL_ICPWM_GPIO_PORT, &GPIO_InitStructure);
}
...
static void TIM_PWMINPUT_Config(void)
{
  ...
  TIM_TimeBaseStructure.TIM_CounterMode=TIM_CounterMode_Up;	
	...
	/* IC1捕获：上升沿触发 TI1FP1 */
  TIM_ICInitStructure.TIM_Channel = GENERAL_IC1PWM_CHANNEL;
  ...
  //TIM_PWMIConfig(ADVANCE_TIM, &TIM_ICInitStructure);
	...
	/* IC2捕获：下降沿触发 TI1FP2 */	
	TIM_ICInitStructure.TIM_Channel = GENERAL_IC2PWM_CHANNEL;
  TIM_ICInitStructure.TIM_ICPolarity = TIM_ICPolarity_Rising;
...
void  GENERAL_TIM_IRQHandler (void)
{
	...
	if(TIM_GetITStatus(GENERAL_TIM,TIM_IT_CC3) != RESET)
	{
		...
		if(CCR3_flag == 0)
		{
			TIM_SetCounter(GENERAL_TIM,0); //计数器清空		从0开始计数高电平持续时间
...
//			printf("IC1RisingValue:%d\r\n",IC1RisingValue);
		...
		{
			IC1FallingValue = TIM_GetCapture3(GENERAL_TIM);	//范围101-202
			
			/*	参考速度给定
			Speed_ref = IC1FallingValue -101;
			*/
			auto_flag = 0;			//失能自动控制
			Speed_ref = (IC1FallingValue -151)*2.0f;
			if(abs(Speed_ref) <= 10 && abs(Ref_Angular_Speed) <= 10)Stop_Control();//0	//AS、S太小，停止控制
			else Set_Loop_Para(Speed_ref,Ref_Angular_Speed);
...
//		}		
	...
	if(TIM_GetITStatus(GENERAL_TIM,TIM_IT_CC4) != RESET)
	{
		...
		{
			IC2FallingValue = TIM_GetCapture4(GENERAL_TIM);
			/*	参考角速度给定
			AngularSpeed_ref = IC2FallingValue -101;
			*/		

			Ref_Angular_Speed = (IC2FallingValue -151)*2.0f;
